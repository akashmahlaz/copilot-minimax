...
import type { TelegramBridge } from '../telegram/telegramBridge';
...
export function registerTelegramTools(context: vscode.ExtensionContext, bridge: TelegramBridge): void {

    // ── telegram_setup ──────────────────────────────────────
    // Allows Copilot to configure the Telegram bot token from chat.

    context.subscriptions.push(
        vscode.lm.registerTool('telegram_setup', {
            async invoke(options: vscode.LanguageModelToolInvocationOptions<{
                ...
                const existingToken = config.get<string>('botToken') || '';
                ...
                try {
                    ...
                    logToolCall('telegram_setup', { botUsername: botName }, `Configured @${botName}`);
    ...
    context.subscriptions.push(
        vscode.lm.registerTool('telegram_status_check', {
            async invoke(_options, _token) {
                ...
                try {
                    ...
                    const botName = data.result?.username || data.result?.first_name || 'unknown';
                    const allowedIds = config.get<number[]>('allowedChatIds') || [];
