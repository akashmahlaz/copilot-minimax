...
function memorySnapshot() {
    ...
    if (memPct < 40 || userPct < 40) {
        const nudges = [];
        if (userPct < 40) {
            nudges.push('Memory is under-utilized. PROACTIVELY use memory_add when you learn: ' +
        ...
        if (memPct < 40) {
            nudges.push('Save environment facts (OS, tools, project structure), conventions, and lessons learned ' +
                'to memory target "memory". Keep entries concise and information-dense.');
        }
        parts.push(`💡 ${nudges.join(' ')}`);
    }
    if (parts.length === 0) {
        return ('💡 Memory is empty. PROACTIVELY use memory_add to save things you learn about the user ' +
